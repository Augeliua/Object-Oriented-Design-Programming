...
public class ManagerUI extends BaseUserUI {
    ...
    protected boolean handleMenuChoice(String choice) {
        switch (choice) {
            ...
            case "9":
                ...
                return false;
            case "10":
                manager.reviewApplications(applicationRepo);
                System.out.println("Applications reviewed successfully.");
                return false;
            case "11":
            ...
            case "12":
                ...
                return false;
            case "13":
                System.out.println(manager.printAllEnquiries(enquiryRepo));
                return false;
            case "14":
                System.out.println(manager.printMyProjectsEnquiries(enquiryRepo));
                return false;
            case "15":
                respondToEnquiry();
                return false;
    ...
    private void toggleProjectVisibility() {
        ...
        System.out.println("Project visibility toggled to: " + (selectedProject.isVisible() ? "Visible" : "Hidden"));
    }
    ...
    private void approveOfficerRegistration() {
        // Get pending officer registrations for projects managed by this manager
        List<HdbOfficer> pendingOfficers = userRepo.getAll().stream()
    ...
    private void handleWithdrawalRequests() {
        ...
        List<Application> withdrawalRequests = applicationRepo.getAll().stream()
                                             .filter(a -> a.isWithdrawalRequested() && 
                                                      manager.getProjectsCreated().stream().anyMatch(p -> 
                                                          p.getProjectID().equals(a.getProject().getProjectID())))
                                             .collect(Collectors.toList());
